...
Since people wanted discord to talk about this (and some even found my personal server) I am putting it here as well (It now has channel for warptorio) Discord Link Here
Interfaces
...
Here is short tutorial how to do it.
    ...
    Install mods that you want this variant to be based on
    Select new single player game and using the sliders for generating new planet create the variant that you want (make sure to check all planets if you are making variant for more that one)
    Start a new game and go to edit mode (/editor)
    Generate all surfaces (Surfaces -> Generate planets)
    Run this command /c for k,v in pairs(game.surfaces) do helpers.write_file(k.."_{your_variant_name}.json",helpers.table_to_json(game.surfaces[k].map_gen_settings)) end 
    locate new variants in .factorio/script-output and transform them from json back to lua
    Make a mod that depends on warptorio
    In your mod add your variants using this remote interface remote.call("warptorio", "edit_planet_variants", variant,planet,map_gen_settings))
