...
local function serialize_ground_platform_design(surface_name)
  if not storage.warptorio then
    return nil, "warptorio_not_initialized"
  end
  local level = storage.warptorio.ground_level or 0
  if level == 0 then
    return nil, "ground_platform_not_unlocked"
  end
  local platform = warp_settings.floor.levels[level]
  if not platform then
    return nil, "ground_platform_level_invalid"
  end
  local surface = game.surfaces[surface_name]
  if not surface then
    return nil, "surface_not_found"
  end
...
function module.on_tick()
  ...
  if not module.has_platforms() then
    return
  end
...
local function roll_position(b_x, b_y)
  ...
  local x = math.random(warp_settings.platforms.position.x.min, warp_settings.platforms.position.x.max) * sign_x
  local y = math.random(warp_settings.platforms.position.y.min, warp_settings.platforms.position.y.max) * sign_y
  return b_x + x, b_y + y
end
...
function module.delete()
  ...
  game.surfaces[storage.warptorio.current_platforms.surface].set_tiles(storage.warptorio.current_platforms.platform)
