...
local space_gen_settings = {
  ...
  property_expression_names = { cliffiness = 0 },
  autoplace_settings = {
    tile = { settings = { ["empty-space"] = { frequency = "normal", size = "normal", richness = "normal" } } },
  },
  starting_area = "none",
}
...
local function get_or_create(name, pos)
  ...
  local exist = game.surfaces[pos.surface].find_entity(name, {
  ...
  return game.surfaces[pos.surface].create_entity({
    name = name,
    position = { x, y },
    direction = pos.dir,
...
local function starter_chest()
  if not warp_settings.starter then
    return
  end
...
script.on_init(function()
  ...
  game.forces.player.set_spawn_position(
    { x = spawn_offset.x, y = spawn_offset.y },
    game.surfaces[storage.warptorio.warp_zone]
  )
  local tiles = generate_surface_rectangle(
    "nauvis",
    warp_settings.floor.levels[1] * 2,
    warp_settings.floor.levels[1] * 2,
...
local function new_random_surface(name)
  ...
  if surface_name == "nauvis" then
    ...
    if
      game.planets[storage.warptorio.surface_name].prototype.entities_require_heating
      or game.planets[storage.warptorio.surface_name].surface ~= nil
    then
...
local function update_belt_biochamber()
  if storage.warptorio.belt_level == 0 then
    return
  end
  ...
  belt_pair(
    { y = 0, x = -5, dir = defines.direction.east, surface = "garden" },
    { y = 0, x = -5, dir = defines.direction.east, surface = "factory" },
    speed[level]
  )
  belt_pair(
    { y = -1, x = -5, dir = defines.direction.east, surface = "garden" },
    { y = -1, x = -5, dir = defines.direction.east, surface = "factory" },
    speed[level]
  )
...
local function update_biochamber_platform(e)
  ...
  local tiles =
    generate_rectangle(platform.width, platform.height, "warp_tile_platform", platform.offset_x, platform.offset_y)
...
local function create_angry_biters(biter_type, number, surface, quality, target)
  ...
  if storage.warptorio.void then
    return
  end
...
local function update_belt(e)
  if storage.warptorio.belt_level == 0 and e == nil then
    return
  end
  local speed = { 15, 30, 45, 60 }
  --game.print("Upgrading belts connection")
  ...
  belt_pair(
    { x = 0, y = -5, dir = defines.direction.south, surface = storage.warptorio.warp_zone },
    { x = 0, y = -5, dir = defines.direction.south, surface = "factory" },
    speed[level]
  )
  belt_pair(
    { x = -1, y = -5, dir = defines.direction.south, surface = storage.warptorio.warp_zone },
    { x = -1, y = -5, dir = defines.direction.south, surface = "factory" },
    speed[level]
  )
  belt_pair(
    { x = 0, y = 4, dir = defines.direction.north, surface = "factory" },
    { x = 0, y = 4, dir = defines.direction.north, surface = storage.warptorio.warp_zone },
    speed[level]
  )
  belt_pair(
    { x = -1, y = 4, dir = defines.direction.north, surface = "factory" },
    { x = -1, y = 4, defines.direction.north, surface = storage.warptorio.warp_zone },
    speed[level]
  )
...
local function check_teleport(player, location, destination)
  ...
  if player.surface.name ~= location.surface then
    return
  end
  local location_pos = translate_surface_position(location.surface, { x = location.x, y = location.y })
  if
    player.position.x > location_pos.x - 0.1
    and player.position.x < location_pos.x + 2.0
    and player.position.y > location_pos.y - 0.1
    and player.position.y < location_pos.y + 2.1
  then
    local dest_pos = translate_surface_position(destination, { x = location.x, y = location.y })
    local player_pos = nil
    if dest_pos then
      player_pos = game.surfaces[destination].find_non_colliding_position(
        "character",
...
local function warp_gui(player)
  ...
  if not storage.warptorio.gui then
    storage.warptorio.gui = {}
  end
  ...
  for _, v in ipairs(elements) do
    if screen_element[v] then
      screen_element[v].destroy()
    end
  end
...
local function update_all_labels()
  ...
  if
    storage.warptorio.planet_next and game.forces["player"].technologies[warp_settings.trigger_research].researched
  then
...
local function technology_check()
  if storage.warptorio and storage.warptorio.transition_timer and storage.warptorio.transition_timer > 60 then
    return false
  end
  ...
  if
    game.forces["player"].current_research.name == "warp-end-prepare"
    or game.forces["player"].current_research.name == "warp-end-win"
  then
    return true
  end
  return false
...
local function spawn_boss_check()
  ...
  if storage.warptorio.wave_index > warp_settings.biter.wave_change_index then
    ...
    if rand > warp_settings.biter.wave_change_chance then
      return true
    end
  end
  return false
...
local function replace_common(entity)
  if not entity.force.name == "enemy" then
    return
  end
  local evolution = get_evolution_factor()
...
local function check_wave()
  ...
  if not storage.warporio.index then
    storage.warporio.index = 0
  end
  if
    not game.forces["player"].technologies["warp-ground-platform-1"].researched
    and game.forces["player"].technologies[warp_settings.trigger_wave].researched == false
    and storage.warporio.index == 0
  then
  ...
  if limit <= 0 then
    ...
    for i = 1, amount do
      if technology_check() or spawn_boss then
        break
      end
...
local function teleport_ground(source, target)
  ...
  local surface_player_list =
    game.surfaces[target].find_entities_filtered({ type = "character", area = destination_area })
...
local function teleport_players(source, destination, factory)
  ...
  local function teleport_to_factory_home(player)
    local home_position = game.surfaces["factory"].find_non_colliding_position("character", { 0, -2 }, 0, 0.5, false)
      or { 0, -2 }
    player.teleport(home_position, destination)
  end

  local function player_on_factory_warp_belt(player)
    if not player.character or player.character.surface.name ~= "factory" then
      return false
    end
  ...
  for i, v in pairs(game.players) do
    ...
    if v.is_player() and v.connected then
      ...
      if factory then
        ...
        teleport_to_factory_home(v)
      elseif
        character_pos.x >= minx
        and character_pos.x <= maxx
        and character_pos.y >= miny
        and character_pos.y <= maxy
      then
        ...
        local target = game.surfaces[destination].find_non_colliding_position(
          v.character,
          fallback_center,
          0,
          platform,
...
local function next_warp_zone_prepare()
  ...
  if
    ...
    and game.forces["player"].technologies["warp-end-prepare"].researched == false
  then
  ...
  if
    ...
    and game.forces["player"].technologies["warp-end-win"].researched == false
  then
    game.print({ "warptorio.technology-cheater" })
    game.forces["player"].technologies["warp-end-win"].saved_progress = 0
  end
  ...
  elseif
    num > 1 - warp_settings.going_home_chance
    and storage.warptorio.surface_name ~= "nauvis"
    and storage.warptorio.warp_next ~= "nauvis"
    and storage.warptorio.void ~= true
  then
...
local function next_warp_zone_finish()
  ...
  if
    storage.warptorio.previous_surface_2 == storage.warptorio.surface_name
    and storage.warptorio.surface_name ~= "nauvis"
    and storage.warptorio.surface_name ~= nil
  then
    keep_time = true
  ...
  local source = nil
  if
    storage.warptorio.factory_level >= warp_settings.space.trigger_factory_level and warp_settings.space.transition
  then
  ...
  if extra_time then
    storage.warptorio.wave_time = storage.warptorio.wave_time + warp_settings.biter.extra_time_amount
  end
  create_void_platform(source, true)
  if
    storage.warptorio.old_surface
    and game.surfaces[storage.warptorio.old_surface]
    and game.surfaces[storage.warptorio.old_surface].valid
  then
...
local function next_warp_zone()
  ...
  if
    storage.warptorio.factory_level >= warp_settings.space.trigger_factory_level and warp_settings.space.transition
  then
    storage.warptorio.transition_timer = math.floor(60 * warp_settings.space.time_per_warp * storage.warporio.index)
...
local function roll_planet()
  ...
  if game.forces["player"].technologies[warp_settings.trigger_research].researched then
    ...
    for i, v in pairs(game.planets) do
      if
        game.forces.player.is_space_location_unlocked(i)
        and i ~= storage.warptorio.surface_name
        and not (i:match(".*%-factory%-floor") or i:match("factory%s-travel%s-surface"))
      then
  ...
  if storage.warptorio.power then
    if battery_check(2) and battery_check(1) then
      ...
      if battery_check(3) then
        ave = (
          storage.warptorio.power[1].energy
          + storage.warptorio.power[2].energy
          + storage.warptorio.power[3].energy
        ) / 3
      end
...
local function update_nauvis_timer()
  if warp_settings.nauvis_timer <= 0 then
    return
  end
  if not storage.warporio or (storage.warporio.index or 0) > 0 then
    return
  end
  ...
  if
    remaining % 60 == 0 or not (storage.warptorio.nauvis_timer_render and storage.warptorio.nauvis_timer_render.valid)
  then
...
script.on_event(defines.events.on_tick, function(event)
  ...
  if not storage.warptorio.transition_timer then
  ...
  if not storage.warptorio.planet_timer then
  ...
  elseif storage.warptorio.warp_out <= 0 and not in_transition_period then
    ...
    if
      ...
      or storage.warptorio.planet_next == nil
      or (storage.warptorio.planet_next == storage.warptorio.warp_zone and storage.warptorio.planet_next ~= "nauvis")
    then
      storage.warptorio.planet_timer = 0
      roll_planet()
    end
  ...
  if
    game.surfaces[storage.warptorio.warp_zone]
    and storage.warptorio.time_passed > warp_settings.polution.time
    and storage.warptorio.warp_zone ~= "nauvis"
  then
...
local function build_entity(e)
  ...
  if e.entity.type == "roboport" then
    ...
    if
      (surface == "factory" and warp_settings.block_roboport_factory)
      or (surface == "garden" and warp_settings.block_roboport_garden)
    then
  ...
  },
  {
    ...
    func = function()
      game.set_win_ending_info({
        title = { "warptorio.end-screen-title" },
        message = {
          "warptorio.end-screen-text",
        },
      })
...
script.on_event(defines.events.on_player_respawned, function(event)
  ...
  local player_pos = surface and surface.find_non_colliding_position("character", spawn_center, 0, 0.5, false)
    or spawn_center
...
script.on_event(defines.events.on_script_trigger_effect, function(event)
  if event.effect_id == "asteroid" then
    if event.source_entity then
      ...
      if
        pos.x < -(size + explosion_size)
        or pos.x > size + explosion_size
        or pos.y < -(size + explosion_size)
        or pos.y > size + explosion_size
      then
        return
      end
...
script.on_event(defines.events.on_train_changed_state, function(e)
  local train = e.train
  if not (train.state == defines.train_state.wait_station and e.old_state == defines.train_state.arrive_station) then
    return
  end
...
commands.add_command(
  "warptorio-set-warp-amount",
  "Set the current warp count (debug). Usage: /warptorio-set-warp-amount <number>",
  function(cmd)
