...
local function on_init_or_load()
  ...
  storage.warptorio.initialized = true
...
script.on_init(function()
  ...
  game.forces.player.set_spawn_position({ x = spawn_offset.x, y = spawn_offset.y },
    game.surfaces[storage.warptorio.warp_zone])
  local tiles = generate_surface_rectangle("nauvis", warp_settings.floor.levels[1] * 2, warp_settings.floor.levels[1] * 2,
...
local function update_belt_biochamber()
  ...
  for i, v in ipairs(speed) do
    if i ~= level then
      local name = "warp-platform-belt-" .. v
      delete_items({ { -5, -1 }, { -4, 1 } }, name, "factory")
      delete_items({ { -5, -1 }, { -4, 1 } }, name, "garden")
    end
  end

  belt_pair({ y = 0, x = -5, dir = defines.direction.east, surface = "garden" },
    { y = 0, x = -5, dir = defines.direction.east, surface = "factory" }, speed[level])
  belt_pair({ y = -1, x = -5, dir = defines.direction.east, surface = "garden" },
    { y = -1, x = -5, dir = defines.direction.east, surface = "factory" }, speed[level])
...
local function update_biochamber_platform(e)
  ...
  local tiles = generate_rectangle(platform.width, platform.height, "warp_tile_platform", platform.offset_x,
    platform.offset_y)
...
local function update_belt(e)
  ...
  belt_pair({ x = 0, y = -5, dir = defines.direction.south, surface = storage.warptorio.warp_zone },
    { x = 0, y = -5, dir = defines.direction.south, surface = "factory" }, speed[level])
  belt_pair({ x = -1, y = -5, dir = defines.direction.south, surface = storage.warptorio.warp_zone },
    { x = -1, y = -5, dir = defines.direction.south, surface = "factory" }, speed[level])
  belt_pair({ x = 0, y = 4, dir = defines.direction.north, surface = "factory" },
    { x = 0, y = 4, dir = defines.direction.north, surface = storage.warptorio.warp_zone }, speed[level])
  belt_pair({ x = -1, y = 4, dir = defines.direction.north, surface = "factory" },
    { x = -1, y = 4, defines.direction.north, surface = storage.warptorio.warp_zone }, speed[level])
...
local function check_teleport(player, location, destination)
  ...
  if player.position.x > location_pos.x - 0.1 and
    ...
    if dest_pos then
      player_pos = game.surfaces[destination].find_non_colliding_position("character", { dest_pos.x, dest_pos.y }, 0, 0.5,
        false) or dest_pos
    end
...
local function update_all_labels()
  ...
  local index = storage.warptorio.index or 0
...
local function replace_common(entity)
  ...
  local quality = choose_quality(storage.warptorio.index)
...
local function check_wave()
  if not game.forces["player"].technologies["warp-ground-platform-1"].researched
      ...
      and storage.warptorio.index == 0 then
  ...
  for i, v in ipairs(warp_settings.biter.tresholds) do
    if v < evolution then
      ...
      if game.forces["enemy"].technologies["warp-weapons-" .. biter_index] then
        if storage.warptorio.index > 50 and warp_settings.dmg_research then
...
local function teleport_players(source, destination, factory)
  ...
  local function teleport_to_factory_home(player)
    local home_position = game.surfaces["factory"].find_non_colliding_position("character", { 0, -2 }, 0, 0.5, false) or
        { 0, -2 }
  ...
  for i, v in pairs(game.players) do
    ...
    if v.is_player() and v.connected then
      ...
      elseif character_pos.x >= minx and character_pos.x <= maxx and character_pos.y >= miny and character_pos.y <= maxy then
        ...
        local target = game.surfaces[destination].find_non_colliding_position(v.character, fallback_center, 0, platform,
          false) or fallback_center
        v.teleport(target, destination)
      end
      ::continue::
    end
...
local function next_warp_zone_prepare()
  ...
  storage.warptorio.index = storage.warptorio.index + 1
  storage.warptorio.time_passed = 0
  local name = "warpzone_" .. storage.warptorio.index
...
local function next_warp_zone_finish()
  ...
  if storage.warptorio.surface_name == "aquilo" then
    ...
    storage.warptorio.warp_out = warp_settings.time.warp_out + storage.warptorio.index * warp_settings.time.add_per_jump
  end
...
local function next_warp_zone()
  ...
  if storage.warptorio.factory_level >= warp_settings.space.trigger_factory_level and
    ...
    storage.warptorio.transition_timer = math.floor(
      60 * warp_settings.space.time_per_warp * storage.warptorio.index)
  ...
  if storage.warptorio.power then
    if battery_check(2) and battery_check(1) then
      ...
      if battery_check(3) then
        ave = (storage.warptorio.power[1].energy + storage.warptorio.power[2].energy + storage.warptorio.power[3].energy) /
            3
      end
...
script.on_event(defines.events.on_tick, function(event)
  if not storage.warptorio or not storage.warptorio.initialized then
    on_init_or_load()
    return
  end
  ...
  if storage.warptorio.ground_level > 0 or
      storage.warptorio.index > 0 then
...
script.on_event(defines.events.on_player_respawned, function(event)
  ...
  local player_pos = surface and surface.find_non_colliding_position("character", spawn_center, 0, 0.5, false) or
      spawn_center
...
commands.add_command("warptorio-set-warp-amount",
  "Set the current warp count (debug). Usage: /warptorio-set-warp-amount <number>", function(cmd)
