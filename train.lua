...
function train_code.warp_array(array, destination, target_station, source_station)
  ...
  for i, v in ipairs(array) do
    ...
    local new_pos = {
      ...
      y = v.position.y - source_station.position.y + target_station.position.y,
    }
...
function train_code.is_station_out_of_bounds(station)
  ...
  if surface_name == "factory" or surface_name == "garden" then
    return false
  end
...
function train_code.warp_trains(train, station_name, destination)
  if not game.forces["player"].technologies["warp-train"].researched then
    return
  end
  if not train then
    return
  ...
  for i, v in ipairs(stations) do
    ...
    if not tmp_train then
      goto next_train_in_loop
    end
    if not tmp_train.valid then
      goto next_train_in_loop
    end
    if not tmp_train.id == train.id then
      goto next_train_in_loop
    end
    ...
    if train_code.is_station_out_of_bounds(v) then
      goto next_train_in_loop
    end
    if train_code.is_station_out_of_bounds(target_station) then
      goto next_train_in_loop
    end
    if train_code.train_has_passengers(train) then
      goto next_train_in_loop
    end
